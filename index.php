...
<head>
    ...
    <style>
        ...
        :root, html[data-theme="ocean"] {
            --primary: linear-gradient(135deg, #BDDFE8, #618698);
            --primary-solid: #618698;
            --secondary: #618698;
            --background: #f0f7f9;
            --card: #ffffff;
            --dark: #030B0D;
        ...
        /* Light Minimal Theme */
        html[data-theme="lightMinimal"] {
            --primary: linear-gradient(135deg, #2563eb, #2563eb);
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .table { margin: 0; }
        .table thead { background: var(--dark-tertiary); }
        .table-hover > tbody > tr { transition: all 0.2s; }
        .table-hover > tbody > tr:hover { --bs-table-hover-bg: var(--hover); cursor: pointer; transform: scale(1.01); }
        .table th { border: none; padding: 15px; font-weight: 600; color: var(--text-primary); }
        ...
        .sortable:hover { background: var(--dark-tertiary); }
        ...
        .logo-preview { width: 32px; height: 32px; object-fit: contain; border: 2px solid var(--border); border-radius: 8px; padding: 5px; cursor: pointer; transition: all 0.3s; }
        .logo-preview:hover { transform: scale(1.1); border-color: var(--primary-solid); }
        .empty-state { text-align: center; padding: 60px 20px; color: var(--text-secondary); }
        .empty-state i { font-size: 4rem; margin-bottom: 20px; opacity: 0.5; }
        .empty-state h4 { margin-bottom: 10px; }
        .evidence-preview { display: inline-block; width: 80px; height: 80px; margin: 5px; border-radius: 8px; overflow: hidden; border: 2px solid var(--border); cursor: pointer; transition: all 0.3s; }
        .evidence-preview:hover { transform: scale(1.1); border-color: var(--primary-solid); }
        ...
        @media print {
            ...
            .document-preview { box-shadow: none; max-width: 100%; margin: 0; padding: 0; background: white; color: #000; }
            .document-preview table thead tr { background-color: var(--dark-tertiary) !important; -webkit-print-color-adjust: exact; color-adjust: exact; }
        }
    </style>
</head>
<body>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <div class="mt-3 p-3 border rounded bg-light">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <label class="form-label mb-0"><strong>Other Services</strong></label>
                                        <button type="button" class="btn btn-sm btn-success" onclick="addOtherServiceItem()">
                                            <i class="fas fa-plus"></i> Add Other Service
                                        </button>
                                    </div>
                                    <div id="otherServicesContainer">
                                        <small class="text-muted">Click "Add Other Service" to add custom service items</small>
                                    </div>
                                </div>
                                ...
                                <script>
                                    ...
                                    let otherServiceCounter = 0;
                                    
                                    // Function to add other service items
                                    function addOtherServiceItem() {
                                        otherServiceCounter++;
                                        ...
                                        serviceDiv.className = 'other-service-item card mb-2';
                                        serviceDiv.dataset.serviceId = otherServiceCounter;
                                        serviceDiv.innerHTML = `
                                            <div class="card-body p-3">
                                                <div class="d-flex justify-content-between align-items-center mb-2">
                                                    <h6 class="mb-0 text-primary">Other Service #${otherServiceCounter}</h6>
                                                    <button type="button" class="btn btn-sm btn-danger" onclick="removeOtherServiceItem(${otherServiceCounter})">
                                                ...
                                                <div class="row">
                                                    <div class="col-12 mb-2">
                                                        ...
                                                        <input type="text" class="form-control other-service-description" 
                                                    ...
                                                    <div class="col-md-6 mb-2">
                                                        ...
                                                        <input type="number" step="0.01" class="form-control other-service-unit-price" 
                                                               data-service-id="${otherServiceCounter}"
                                                               required min="0" 
                                    ...
                                    // Function to remove other service items
                                    function removeOtherServiceItem(serviceId) {
                                        const item = document.querySelector(`.other-service-item[data-service-id="${serviceId}"]`);
                                        if (item) {
                                            item.remove();
                                            ...
                                            if (container.children.length === 0) {
                                                container.innerHTML = '<small class="text-muted">Click "Add Other Service" to add custom service items</small>';
                                            }
                                            updateItemAmounts();
                                        }
                                    }
                                    ...
                                    function updateItemAmounts() {
                                        ...
                                        const otherServiceItems = document.querySelectorAll('.other-service-item').length;
                                        const count = selectedItems.length + otherServiceItems;
                                    ...
                                    // Expose updateItemAmounts globally for use when opening modal
                                    window.updateItemAmounts = updateItemAmounts;
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" onclick="printCurrentDocument()"><i class="fas fa-print me-2"></i>Print</button>
    ...
    <script>
        ...
        function getTheme() {
            return localStorage.getItem('ayonion_theme') || 'ocean';
        }
            ...
            const reportHtml = `
                <div style="font-family: Arial; padding: 20px;">
                    <div style="display: flex; align-items: center; margin-bottom: 30px; border-bottom: 2px solid #e5e7eb; padding-bottom: 20px;">
                        ${client.logoUrl ? `<img src="${client.logoUrl}" style="width: 48px; height: 48px; object-fit: contain; margin-right: 20px; border-radius: 8px;">` : ''}
                        <div>
        ...
        function initDocumentForm() {
			...
			documentForm.addEventListener('submit', async function(e) {
                ...
                // Check if all required elements exist and at least one item type is selected
				if (!clientSelect || !date) {
                    console.error('One or more required form elements not found');
                    showAlert('Form error: Missing required form elements', 'danger');
                    return;
                }
                ...
                const dynamicInputs = form.querySelectorAll('.item-quantity, .item-unit-price, .item-description');
                for (let input of dynamicInputs) {
                    if (input.classList.contains('item-description')) {
                        // Validate description for Other Service
                ...
                // Handle "Other Service" items separately (multiple allowed)
                const otherServiceItems = document.querySelectorAll('.other-service-item');
                otherServiceItems.forEach(serviceItem => {
                    ...
                    if (descriptionInput && quantityInput && unitPriceInput) {
                        ...
                        if (!description) {
                            validationError = {
                                message: 'Description is required for all "Other Service" items.',
                ...
                if (itemDetails.length === 0) {
                    showAlert('Please add at least one item with valid quantities and prices.', 'warning');
                    return;
                }
                
                // Extract descriptions from "Other Service" items if present (combine multiple)
                let documentDescription = '';
                const otherServiceDescriptions = itemDetails
                    .filter(item => item.itemType === 'Other Service' && item.description)
                    .map(item => item.description);
                if (otherServiceDescriptions.length > 0) {
        ...
        // ✅ Print current document preview (for content reports)
        function printCurrentDocument() {
            const documentPreview = document.getElementById('documentPreview');
            if (!documentPreview || !documentPreview.innerHTML.trim()) {
                showAlert('No document to print.', 'warning');
                return;
            }
